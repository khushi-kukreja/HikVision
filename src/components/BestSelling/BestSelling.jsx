    ...
    return (
        <div className="flex flex-col mt-8 sm:mt-12 px-4 sm:px-6 lg:px-10 w-full max-w-[90vw] mx-auto">
            ...
            <div className="flex flex-wrap justify-between items-center w-full gap-4">
                <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold font-inria">
                ...
                <div className="hidden sm:flex gap-2">
                    <button
                        onClick={scrollLeft}
                        className="border border-black p-1 sm:p-2 w-7 h-7 sm:w-8 sm:h-8 flex items-center justify-center"
                    >
                        <img src={assets.left_arrow} alt="btn" className="w-full h-full" />
                    </button>
                    <button
                        onClick={scrollRight}
                        className="border border-black p-1 sm:p-2 w-7 h-7 sm:w-8 sm:h-8 flex items-center justify-center"
                    >
                        <img src={assets.right_arrow} alt="btn" className="w-full h-full object-cover" />
                    </button>
                </div>
            </div>
            ...
            {showAll ? (
                <div className="grid grid-cols-[repeat(auto-fit,minmax(150px,1fr))] sm:grid-cols-[repeat(auto-fit,minmax(200px,1fr))] md:grid-cols-[repeat(auto-fit,minmax(240px,1fr))] mt-7 gap-4 sm:gap-6 lg:gap-8">
                    {trending.map((item, index) => (
                        <div
                            key={index}
                            className="shadow-[0_0_10px_rgba(0,_0,_0,_0.5)] p-3 sm:p-4 rounded-md text-center bg-white"
                        >
            ...
            <div className="flex justify-end">
                <button
                    ...
                    className="transition-all duration-300 p-1 sm:p-2 mt-4 border border-black bg-[#d9d9d9] w-[90px] sm:w-[100px] text-sm sm:text-base"
                >
